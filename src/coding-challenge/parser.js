...
function parseTransaction(o, index) {
  return Validated_1.obj({
    ...
    deltaContraband: Validated_1.alternative(
      parseUndefined(o, 'deltaContraband', index),
      parseNumber(o, 'deltaContraband', index)
    ),
    shipPurchase: Validated_1.alternative(
      parseUndefined(o, 'shipPurchase', index),
      parseShipPurchase(o, 'shipPurchase', index)
    ),
  });
}
function parse(json) {
  return Validated_1.flatMap(parseArray(json), arr => {
    ...
    const validatedTransactions = validatedObjects.map((vo, index) =>
      Validated_1.flatMap(vo, o => parseTransaction(o, index))
    );
