...
function generatePlanets(seed, numberOfPlanets) {
    ...
    for (let i = 0; i < numberOfPlanets; ++i) {
        ...
        const waterPrice = random_1.toIntegerBetween(MIN_PRICE, MAX_PRICE, random());
        const partsPrice = random_1.toIntegerBetween(MIN_PRICE, MAX_PRICE, random());
        const factionNumber = random();
        const faction = factionNumber < 0.5 ? "Liberty" : "Black Moranth";
        const contrabandPrice = faction === "Black Moranth"
            ? random_1.toIntegerBetween(MIN_PRICE * CONTRA_BAND_FRACTION, MAX_PRICE * CONTRA_BAND_FRACTION, random())
            : 0;
        planets.push({
            orePrice,
            waterPrice,
